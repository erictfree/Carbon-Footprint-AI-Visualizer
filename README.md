...
Burger Works is a one-round carbon comparison game. A cartoon setup board collects a model, typical output, daily prompt rate, and Masley lifestyle defaults; then two burger-production lines turn the computed 30-day footprints into a synchronized 48.8-second factory run.
...
The AI estimate uses the model curves transcribed from [Andy Masley’s public calculator source](https://andymasley.com/visuals/ai-prompt-footprint-source.txt), based on EcoLogits v0.10. Output-token scenarios are interpolated per model and request.
