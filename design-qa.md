...
- Pass — the new background's horizon divider and lifestyle-belt fan differ from Burger Belt 2. Packed left rows receive a −2.4% rear center correction; packed right rows receive −3.8%, and both fade to zero before the foreground. The stronger right correction is localized near the vanishing point, where the photographed lane is narrow; the right base path still runs from 55.2% at the rear to 80.8% at the foreground, preserving the accepted front alignment.
