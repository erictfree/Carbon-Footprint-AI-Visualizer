...
- Pass — the LED overlays were remeasured against the new photographed recesses: both use 9.2% stage width at 41.8% stage height, with the left inset at 12.65% and right inset at 12.85%. Each recess is now a single padded CO₂ counter—pace/day captions were removed so the authoritative accumulated value can use the largest responsive type that safely fits the black window.
