...
- Pass — same-row spacing is derived from each sprite's rendered width plus a fixed one-percent stage gap. Desktop horizontal gaps held between 11.5 and 12.0 px from the distant row through the foreground; compact gaps held between 3.63 and 3.65 px. The distant scale was doubled from 0.07 to 0.14 so the first burgers are readable without changing the established foreground size.
