...
- Pass — the AI stream follows a 49.4% → 30% centerline. The rear origin stays aligned with the annotated ramp mouth, while the farther-right foreground endpoint rotates the inner burger ray to run parallel with the white divider guide instead of peeling left as it approaches the camera. In three-wide rows the accepted outer-left ray stays fixed and the two remaining rays use equal 0.92-column gaps, pulling the inner burger clear of the gold divider rail without fanning the row.
