...
- Pass — the new background's horizon divider and lifestyle-belt fan differ from Burger Belt 2. Packed AI rows now use their calibrated centerline directly; the legacy −2.4% rear translation was removed because it pushed the outer rear burger off the new ramp. Packed lifestyle rows use a 54.75% → 80.35% centerline, receive a final 10 px leftward stage correction, and compress only their distant column spread to 56%, interpolating to full physical spacing at the foreground.
- Pass — burger scale runs from 0.10 at the horizon to 1.18 at the photographed foreground edge, with the foreground edge gap held at 1.5% of stage width. This makes the rear silhouettes proportionally smaller while preserving the accepted foreground presence and clearance; column spacing derives from the same endpoints and remains proportional to the rendered burgers.
- Pass — the AI stream follows a 49.4% → 30% centerline. The rear origin stays aligned with the annotated ramp mouth, while the farther-right foreground endpoint rotates the inner burger ray to run parallel with the white divider guide instead of peeling left as it approaches the camera. In three-wide rows the accepted outer-left ray stays fixed and the two remaining rays use equal 0.68-column gaps, so the two visible outside edges track the annotated belt guides without fanning the row.
- Pass — the LED overlays were remeasured against the new photographed recesses: both use 9.2% stage width. The cyan counter sits at 42.65% stage height with a 13.35% left inset; the amber counter sits at 43.3% with a 9.85% right inset, centered on the photographed black window. Each recess is a single padded CO₂ counter—pace/day captions are removed, and each value refits from its exact rendered text rather than character count, leaving safe black-window padding as digits change.

## AI outside-edge guide pass

- Source visual truth: `artifacts/design-qa/ai-guide-ray-reference.png` (1546 × 1250 px), the user's zoomed AI-belt crop with two white perspective guides.
- Browser-rendered implementation: `artifacts/design-qa/ai-guide-ray-fit.png` (871 × 1133 px), captured from the in-app Browser at an 871 × 1133 CSS viewport and 1× output density.
- State: Agent marathon preset, packed three-wide AI rows, default lifestyle line, nine seconds into the synchronized batch.
- Normalization: the source is a user-created zoom/crop rather than a full-page viewport. The comparison therefore normalizes against the photographed belt plane and judges the two outside silhouette rays at rear, middle, and foreground depths rather than treating raw page pixels as 1:1 coordinates.
- Full-view comparison evidence: the implementation capture confirms that the entire arcade shell remains intact, the AI stream stays on the left belt, and no surrounding layout or interaction changed.
- Focused-region comparison evidence: the annotated crop and implementation were opened together. The earlier P2 mismatch was excessive AI row width—the accepted outer-left edge tracked its guide while the middle and right burgers fanned across the inner guide. The fix keeps the left coordinate at −1 and changes the two equal column steps from 0.92 to 0.68 (`[-1, -0.32, 0.36]`), pulling both inner rays inward without translating or curving the accepted outside ray. The post-fix rear, middle, and foreground rows remain parallel and clear both rails; no actionable P0/P1/P2 mismatch remains.
- Required fidelity surfaces: fonts, copy, colors, raster image quality, stage crop, and surrounding spacing are unchanged from the passed arcade-interface review. Only projective burger spacing changed, and the supplied burger raster remains unscaled beyond the existing perspective transform.
- Primary interaction check: the setup dialog opened, Agent marathon was selected, Done started the round, and the three-wide rows plus LED counters advanced together. The existing automated suite and production build remain clean.
