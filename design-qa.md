...
- Pass — one selected comparison window maps to one playback minute. Exact output sets launch cadence; each belt fills aligned rows across its width before accelerating. In the typical 718× state, the persistent AI marker takes 60 seconds to traverse while the denser three-wide lifestyle rows take about 4.75 seconds.
...
- Pass — the user-supplied screenshot calibrates single-burger belt centers independently from packed-row paths: 43.4%/56.6% at the back and 22.3%/77.7% at the foreground. Packed three-wide rows retain their separate 0.5% outward rail-clearance nudge, so a sparse marker stays centered for its full traversal without moving the dense comparison line.
- Pass — burger bottoms, rather than sprite centers, are anchored to the plane. The populated desktop line carries eight aligned rows of three, with 18–21 burgers intersecting the visible stage and a measured minimum full-bounding-box clearance of 6.9 px across 2.4 seconds with zero intersections. Frame-by-frame tracking showed every sampled following distance widening, eliminating the apparent rear-row catch-up. Compact rows run two-wide with at least 3.6 px clearance and zero intersections.
- Pass — same-row spacing is derived from each sprite's rendered width plus a rail-safe gap that opens with the photographed belt. Desktop horizontal gaps grow from about 6 px at the distant row to 11–12 px at the foreground; compact gaps grow from 1.82 to 3.64 px. The distant scale was doubled from 0.07 to 0.14 so the first burgers are readable without changing the established foreground size.
...
- Live desktop-to-compact resize: passed without reload. The `ResizeObserver` changed the populated-line capacity from twenty-four three-wide burgers to six two-wide burgers, then restored the desktop arrangement.
...
- Restart lines: passed; the typical state immediately seeds one slow AI marker and eight aligned rows of three lifestyle burgers on desktop (three rows of two on compact screens) before continuous cadence resumes.
...
- Dense-balance pass — P1 resolved. The six-row desktop cap left avoidable foreground space on the high-output line. The cap is now eight synchronized rows of three: 18–21 burgers remain on-screen in the typical state, exact cadence is unchanged, and belt travel slows to about 4.75 seconds before any further rate increase is represented as velocity.
