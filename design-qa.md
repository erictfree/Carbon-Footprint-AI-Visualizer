...
- Pass — one selected comparison window maps to one playback minute. Exact output sets launch cadence; each belt fills aligned rows across its width before accelerating. In the typical 718× state, the persistent AI marker takes 60 seconds to traverse while the three-wide lifestyle rows take about 3.53 seconds.
...
- Pass — burger bottoms, rather than sprite centers, are anchored to the plane. The populated desktop line carries six aligned rows of three, with 15–18 burgers visible and a measured minimum full-bounding-box clearance of 6.5 px across 2.4 seconds with zero intersections. Frame-by-frame tracking of every consecutive row showed its visible following distance increasing monotonically from the vanishing point through the foreground, eliminating the apparent rear-row catch-up. Compact rows run two-wide with zero intersections.
...
- Live desktop-to-compact resize: passed without reload. The `ResizeObserver` changed the populated-line capacity from eighteen three-wide burgers to six two-wide burgers, then restored the desktop arrangement.
...
- Restart lines: passed; the typical state immediately seeds one slow AI marker and six aligned rows of three lifestyle burgers on desktop (three rows of two on compact screens) before continuous cadence resumes.
...
- Perspective-headway pass — P1 resolved. Sprite scale previously used much stronger perspective than forward position, so rear burgers appeared to consume the gap even though their centers never passed. Position, scale, column spread, and the below-frame exit now share one continuous belt-plane projection. Every tracked following-distance sequence increased frame by frame; desktop and compact stress sampling retained zero intersections.
