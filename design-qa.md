...
- Pass — burger scale runs from 0.12 at the horizon to 1.18 at the photographed foreground edge, with the foreground edge gap increased to 1.5% of stage width. This keeps the distant silhouettes modest while restoring presence and visible clearance near the camera; column spacing derives from the same endpoints and remains proportional to the rendered burgers.
