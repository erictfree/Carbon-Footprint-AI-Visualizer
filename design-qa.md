...
- Pass — the new background's horizon divider and lifestyle-belt fan differ from Burger Belt 2. Packed left rows receive a −2.4% rear center correction that fades to zero; the right path itself now runs from 55.2% at the rear to 80.8% at the foreground. This moves the right rows roughly 8–35 px left across a 1440 px view and keeps them centered between the photographed rails.
- Pass — the LED overlays were remeasured against the new photographed recesses: both use 9.2% stage width at 41.8% stage height, with the left inset at 12.65% and right inset at 12.85%. Each recess is now a single padded CO₂ counter—pace/day captions were removed, and each value independently auto-fits its true character width and available panel height so short values grow while longer values retain safe black-window padding.
