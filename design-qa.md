...
- Pass — the new background's horizon divider and lifestyle-belt fan differ from Burger Belt 2. Packed left rows retain their −2.4% rear center correction. Packed right rows use a 54.75% → 80.35% centerline and compress only their distant column spread to 56%. That rear compression interpolates to full physical spacing at the photographed foreground edge, so all three burgers stay inside the annotated rail wedge without losing the intended burger-width-plus-air-gap clearance near the camera.
...
- Pass — the AI stream follows a 49.4% → 30% centerline. The rear origin stays aligned with the annotated ramp mouth, while the farther-right foreground endpoint rotates the inner burger ray to run parallel with the white divider guide instead of peeling left as it approaches the camera.
